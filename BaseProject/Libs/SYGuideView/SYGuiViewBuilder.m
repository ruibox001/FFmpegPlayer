...
/**
 引导页实现类
 */
@property (nonatomic,strong) SYGuideViewController *guideView;
...
GuiViewBuilder *guiViewBuilder(){
    ...
    if (!m) {
        ...
        m.guideView = [[SYGuideViewController alloc] init];
    }
...
/**
 ...
 @return 构建器
 */
- (GuiViewBuilder *(^)(BOOL rootViewController))show {
    return ^id(BOOL rootViewController){
        [self buildGuiView:rootViewController];
        return self;
    };
}
...
- (void)buildGuiView:(BOOL)rootViewController {
    if (!self.guideView) {
        self.guideView = [[SYGuideViewController alloc] init];
    }
    [self.guideView show:rootViewController];
}
